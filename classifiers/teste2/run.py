...
options = json.load(open("teste2/variables.json", "r"))
...
dataHandler.readDataFile(DATAFILE,maxlen,max_features)
...
# model = Sequential()
# embedding = Embedding(max_features, embedding_size, input_length=maxlen)
# embedding.trainable = train_embedding
# model.add(embedding)
# model.add(Dropout(0.25))
# conv1 = Sequential()
# conv1.add(Conv1D(filters,
#                  3,
#                  padding='valid',
#                  activation='relu',
#                  strides=1))
# conv1.add(MaxPooling1D(pool_size=pool_size))

# conv2 = Sequential() 
# conv2.add(Conv1D(filters,
#                  4,
#                  padding='valid',
#                  activation='relu',
#                  strides=1))
# conv2.add(MaxPooling1D(pool_size=pool_size))

# conv3 = Sequential() 
# conv3.add(Conv1D(filters,
#                  5,
#                  padding='valid',
#                  activation='relu',
#                  strides=1))
# conv3.add(MaxPooling1D(pool_size=pool_size))
# # model.add(Conv1D(filters,
# #                  kernel_size,
# #                  padding='valid',
# #                  activation='relu',
# #                  strides=1))
# model.add(Concatenate([conv1,conv2,conv3]))
...
with open("teste2/results/out_"+current_time+".json", "w") as outfile:
